<script lang="ts">
    ...
    function addProfession(name: string) {
        if (!name.trim()) return;
        character.update(c => ({
            ...c,
            professions: [...(c.professions || []), name.trim()]
        }));
    }

    function removeProfession(index: number) {
        character.update(c => {
            const newProfessions = [...(c.professions || [])];
...
<div class="bg-slate-900 rounded-2xl border border-slate-800 p-4 shadow-lg shadow-black/20">
    ...
    <div class="flex flex-wrap gap-2 mb-4">
        {#each ($character.professions || []) as prof, idx}
            <div class="bg-slate-950 border border-slate-700 text-slate-200 px-3 py-1.5 rounded-xl text-sm font-bold flex items-center gap-2 group transition-all hover:border-indigo-500/50">
                ...
                <button
                    ...
                    class="text-slate-500 hover:text-red-400 p-1 rounded-full hover:bg-red-400/10 transition-all font-normal ml-1"
