<script lang="ts">
   ...
   import { Users, LogOut, Wifi, WifiOff, Clock } from 'lucide-svelte';
...
{#if $sotdlCharacter.campaignId}
    {@const isPending = $sotdlCharacter.campaignApproval === 'pending'}
    {#if banner}
        ...
        <div class="bg-indigo-600/10 backdrop-blur-md border border-indigo-500/20 rounded-2xl p-4 flex items-center justify-between shadow-xl shadow-indigo-950/20 {isPending ? 'border-amber-500/30 bg-amber-500/5' : ''}">
            <div class="flex items-center gap-3">
                ...
                <div>
                     ...
                     <div class="font-bold text-white text-base leading-tight">{$sotdlCharacter.campaignName}</div>
                     {#if isPending}
                     ...
                     {:else}
                        <div class="text-[10px] text-slate-400 mt-0.5 font-medium flex items-center gap-1">
                            ...
                            <span class="text-indigo-300">{$sotdlCharacter.gmName}</span>
                        </div>
                     {/if}
                </div>
            </div>
            <button
                onclick={() => { if(confirm($t('campaign.leave_confirm'))) sotdlCharacterActions.leaveCampaign() }}
    ...
    {:else if compact}
        <div class="inline-flex items-center gap-2 px-3 py-1 {isPending ? 'bg-amber-500/10 border-amber-500/20' : 'bg-indigo-500/10 border-indigo-500/20'} rounded-full shadow-sm glass">
            ...
            <span class="text-[9px] font-black {isPending ? 'text-amber-500' : 'text-indigo-400'} uppercase tracking-widest truncate max-w-[150px]">{$sotdlCharacter.campaignName}</span>
        </div>
    {:else}
        ...
        <div class="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-lg backdrop-blur-sm group hover:border-indigo-500/30 transition-all duration-300 {!$isGmOnline && !isPending ? 'opacity-60 saturate-50' : ''} {isPending ? 'border-amber-500/20' : ''}">
            <div class="flex items-center justify-between">
                ...
                <button
                    onclick={sotdlCharacterActions.leaveCampaign}
            ...
            <div class="space-y-1">
                 <div class="font-bold text-white text-lg tracking-tight leading-tight">{$sotdlCharacter.campaignName}</div>
                 <div class="text-[10px] font-medium text-slate-500 flex items-center gap-1.5 pt-0.5">
                    ...
                    <span class="text-indigo-400 font-bold">{$sotdlCharacter.gmName}</span>
                 </div>
            </div>
