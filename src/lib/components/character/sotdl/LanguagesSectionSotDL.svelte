<script lang="ts">
    ...
    const { addLanguage, removeLanguage } = sotdlCharacterActions;
...
<div class="bg-slate-900 rounded-2xl border border-slate-800 p-4 shadow-lg shadow-black/20">
    <div class="flex justify-between items-center mb-4">
        <h4 class="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
            <Languages size={14} class="text-indigo-500"/> {$t('character.languages.title')}
        </h4>
    </div>

    <div class="flex flex-wrap gap-2 mb-4">
        {#each $sotdlCharacter.languages as lang, idx}
        ...
        {/each}
        {#if $sotdlCharacter.languages.length === 0}
            <span class="text-xs text-slate-500 italic py-1 px-1">{$t('character.languages.none')}</span>
        {/if}
    </div>
