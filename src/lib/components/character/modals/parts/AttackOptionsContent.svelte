<script lang="ts">
    ...
    function confirmRoll() {
        ...
        const rollData = {
            type: rollDataType,
            source: data.source || data, // Handle source if nested or direct
            ...
            <div class="space-y-6 pt-2">
                ...
                <div class="p-4 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-between">
                     ...
                     <span class="font-bold text-white">
                        {rollType === 'attack' ? 'Ataque (1d20)' : rollType === 'damage' ? `Dano (${data.damageDice})` : rollType === 'luck' ? 'Sorte (1d20)' : `Teste (${data.label || '1d20'})`}
                     </span>
                </div>
