<script lang="ts">
    ...
    import { Plus, Edit, Zap, Infinity as InfinityIcon, Clock, Hourglass } from 'lucide-svelte';
    import { TALENT_SOURCES, DURATION_TYPES } from '../../../routes/sofww';

    const { recoverTalent, rollLuckTalent } = characterActions;
    ...
    {#if hasAnyTalent}
        {#each Object.values(TALENT_SOURCES) as source}
            {#if groupedTalents[source] && groupedTalents[source].length > 0}
                <div class="space-y-4">
                    <h4 class="text-sm font-black text-slate-500 uppercase tracking-widest border-b border-slate-800 pb-2">
                        {$t(`character.talents.sources.${source}`)}
                    </h4>
                    <div class="grid grid-cols-1 gap-4">
                        {#each groupedTalents[source] as talent}
                            <div class="p-5 bg-slate-900/40 backdrop-blur-sm border border-slate-800 rounded-2xl flex flex-col md:flex-row md:items-center gap-6 group hover:border-indigo-500/50 hover:bg-slate-900/60 transition-all duration-300 {talent.activityType !== 'Passive' && !talent.isPassive && talent.uses === 0 ? 'opacity-60 grayscale-[0.5]' : ''}">
                                <div class="flex-1">
                                ...
                                <div class="shrink-0 flex items-center gap-4 bg-slate-950/50 p-3 rounded-xl border border-slate-800 shadow-inner">
                                   <div class="text-center px-2 border-r border-slate-800 pr-4 min-w-[80px]">
                                      ...
                                      {:else if talent.activityType === 'Duration'}
                                          <span class="block text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">{$t('character.talents.types.Duration')}</span>
                                          <div class="font-bold text-indigo-300 text-xs flex flex-col items-center justify-center bg-slate-800 px-2 py-1 rounded-md">
                                              {#if talent.duration === 'LUCK_ENDS'}
                                                  <span class="flex items-center gap-1"><Clock size={12}/> Sorte Encerra</span>
                                              {:else}
                                                  <span class="flex items-center gap-1"><Hourglass size={12}/> {talent.durationValue} {talent.duration === 'MINUTES' ? 'min' : 'h'}</span>
                                              {/if}
                                          </div>
                                      {:else}
                                   ...
                                   {#if talent.activityType !== 'Passive' && !talent.isPassive}
                                       <div class="flex gap-2">
                                           {#if talent.activityType === 'Uses'}
                                            ...
                                            {:else if talent.activityType === 'Duration' && talent.duration === 'LUCK_ENDS' && talent.uses === 0}
                                                <button 
                                                    onclick={() => rollLuckTalent(talent.id)} 
                                                    title="Tentar Recuperar (Teste de Sorte)"
                                                    class="bg-slate-800 hover:bg-yellow-600/20 text-yellow-500 p-2 rounded-lg transition-all hover:scale-110 active:scale-90 border border-slate-700 hover:border-yellow-500/50 flex flex-col items-center gap-0.5"
                                                    aria-label="Recuperar {talent.name} com sorte"
                                               >
                                                    <Clock size={18} />
                                                    <span class="text-[8px] font-black uppercase">Recuperar</span>
                                               </button>
                                            {/if}
                                           <button 
